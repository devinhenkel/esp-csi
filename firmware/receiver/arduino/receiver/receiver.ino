/**
 * ESP-CSI Receiver — Arduino IDE version
 * Target: ESP32-S3 or ESP32-C6 (CSI support required)
 *
 * Arduino IDE setup:
 *   1. Install "esp32 by Espressif Systems" ≥ 2.0.0 via Boards Manager
 *   2. Select board:
 *        ESP32-S3: Tools → Board → ESP32 Arduino → ESP32S3 Dev Module
 *        ESP32-C6: Tools → Board → ESP32 Arduino → ESP32C6 Dev Module
 *   3. Open this folder (File → Open → select this .ino)
 *   4. Edit the config section below
 *   5. Sketch → Upload
 *
 * The CSI capture uses the underlying esp_wifi API, which is accessible
 * from within the Arduino ESP32 framework via esp_wifi.h.
 */
...
#define BRIDGE_HOST    "192.168.1.100"   // IP of machine running bridge/main.py
#define BRIDGE_PORT    8765
#define BRIDGE_PATH    "/ingest/rx"

// Sliding window size in CSI frames — larger = smoother but higher latency
#define FEATURE_WINDOW 20

// Transmitter MAC filter: "AA:BB:CC:DD:EE:FF" or "" to accept all
#define TX_MAC_FILTER  ""

// ── Ring buffer ───────────────────────────────────────────────────────────
#define RING_SIZE    64
#define MAX_CSI_LEN  384   // covers LLTF + HT-LTF at 40 MHz
...
// ── CSI callback (runs in WiFi task — keep it minimal) ───────────────────
static void IRAM_ATTR wifiCsiCb(void *ctx, wifi_csi_info_t *csi) {
  ...
  if (next == ringTail) {          // buffer full — overwrite oldest
    rxLost++;
...
// ── CSI init (must be called after WiFi is connected) ────────────────────
static void csiInit(void) {
...
// ── Drain ring buffer and run feature pipeline ────────────────────────────
void processRingBuffer(void) {
  while (true) {
    portENTER_CRITICAL(&ringMux);
