/**
 * ESP-CSI Transmitter — Arduino IDE version
 ...
 *   4. Edit WIFI_SSID / WIFI_PASSWORD below
 *   5. Sketch → Upload
 *
 * Serial commands (115200 baud):
 *   set_rate <hz>     packet rate (1–500)
 *   set_channel <n>   logical channel (1–13)
 *   start_tx          resume transmission
 *   stop_tx           pause transmission
 *   status            print telemetry immediately
 */
...
void loop() {
  ...
  // Send TX packet at configured rate
  ...
  // Serial command parser (line-buffered)
...
void handleCommand(const char *line) {
  ...
  if (strncmp(line, "set_rate ", 9) == 0) {
    ...
    lastHbMs = 0;  // triggers immediate heartbeat on next loop()
  } else {
